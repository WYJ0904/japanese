...
词表页提供“AI 联网选词”：日语支持 JLPT N5 至 N1，英语支持小学三至六年级、初中一至三年级、高中一至三年级及大学英语四、六级。用户可填写数量并选择替换或追加词表。追加时前端会把已有词传给后端排除，后端也会再次过滤重复词；界面按实际新增数量反馈，不会把重复词误报为已追加。后端只访问预设搜索服务，日语优先分页读取带 JLPT 标签的候选和假名读音，英语按最多 50 个一批调用本地 Ollama 补齐；外网资料暂不可用时会明确降级为本地 AI。普通账户仍受 15 词限制，其他账户单次最多生成 200 词。

日语听写中，纯假名词按原词形作答；含汉字的词必须同时填写汉字词形和假名读音，例如 `学校 / がっこう`。联网生成的日语词表会自动保存读音；手工词表也可写成 `学校｜がっこう`。旧版仅含单词的词表仍可读取，开始听写时后端会为缺失的汉字词批量补充读音。空答案会显示输入提示并停留在当前题，不会判错或写入错题本。
...
英语和日语的判卷方式、练习方式与 AI 选词选项分别保存在新增的 `gradingMode:english|japanese`、`practiceMode:english|japanese` 和 `aiSuggestSettings:english|japanese` 键中。日语汉字词读音保存在新增的 `japaneseReadingCache:v1` 中；这是独立的可再生成缓存，不改变旧词表或错题结构。首次升级会从旧版共享设置迁移一份初始值，之后两个项目互不串台；原有键仍保留用于向旧版本兼容。登录成功和退出登录都会清空页面中的明文密钥输入框。
